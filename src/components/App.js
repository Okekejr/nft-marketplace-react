...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    if (networkData === networkId) {
      ...
      window.alert(
        "Connect to Ganache network manually on metamask to load contracts"
      );
    }
  }
  ...
  async loadWeb3() {
    ...
    if (provider) {
      console.log("desktop has walletprovider");
      window.web3 = new Web3(provider);
      await this.loadBlockchainData()
    } else {
      // no ethereum provider
      console.log("no walletprovider detected");
    }
  }
    ...
    return (
      <div className="container-filled">
        ...
        >
          ...
          <button className="mr-5" onClick={this.openWallet.bind(this)}>Connect</button>
        </nav>
